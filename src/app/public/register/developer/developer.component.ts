...
export class DeveloperComponent implements OnInit {
  ...
  AddProgrammingLanguages() {
    for (let i = 0; i < this.registerForm.get('languages')?.value.length; i++) {
      ...
      this.language.description =
        'I am expert with programming language ' +
        this.registerForm.get('languages')?.value.at(i);
      if (this.language.name === 'JavaScript') {
        this.language.iconLink =
        ...
        this.language.iconLink =
          'https://e7.pngegg.com/pngimages/383/488/png-clipart-ruby-on-rails-programming-language-computer-programming-logo-ruby-ruby-scripting-language-thumbnail.png';
      }
      this.service
        .postLenguage(this.language, this.digitalProfile.id)
  ...
  AddDatabases() {
    for (let i = 0; i < this.registerForm.get('databases')?.value.length; i++) {
      ...
      this.database.description =
        'I am expert with database' +
        this.registerForm.get('databases')?.value[i];
      if (this.database.name === 'MySQL') {
        this.database.iconLink =
          'https://pngimg.com/uploads/mysql/mysql_PNG22.png';
      } else if (this.database.name === 'Oracle') {
        this.database.iconLink =
          'https://www.iconshock.com/image/Stroke/Database/oracle/';
      } else if (this.database.name === 'PostgreSQL') {
        this.database.iconLink =
          'https://upload.wikimedia.org/wikipedia/commons/2/29/Postgresql_elephant.svg';
      } else if (this.database.name === 'Microsoft SQL Server') {
        this.database.iconLink =
          'https://i.pinimg.com/originals/ae/09/c5/ae09c54080a6ae50d58459ea3193835c.png';
      } else {
        this.database.iconLink = 'https://logodix.com/logo/2090013.jpg';
      }
      this.service
        .postDatabase(this.database, this.digitalProfile.id)
  ...
  AddFrameworks() {
    for (
      let i = 0;
      i < this.registerForm.get('frameworks')?.value.length;
      i++
    ) {
      this.frameWork.name = this.registerForm.get('frameworks')?.value[i];
      this.frameWork.description =
        'I am expert with framework ' +
        this.registerForm.get('frameworks')?.value[i];
      if (this.frameWork.name === 'Angular') {
        this.frameWork.iconLink =
          'https://upload.wikimedia.org/wikipedia/commons/c/ca/AngularJS_logo.svg';
      } else if (this.frameWork.name === 'ASP.NET Core') {
        this.frameWork.iconLink =
          'https://ih0.redbubble.net/image.366684650.5673/flat,750x,075,f-pad,750x1000,f8f8f8.u1.jpg';
      } else if (this.frameWork.name === 'Django') {
        this.frameWork.iconLink =
          'https://th.bing.com/th/id/OIP.XltbHSe7XNacUtF12tymrgHaCl?pid=ImgDet&rs=1';
      ...
      this.service
        .postFramework(this.frameWork, this.digitalProfile.id)
        .subscribe((response: any) => {
          console.log(response);
        });
    }
  ...
  GetDigitalProfile() {
    this.service
      .getDigitalProfileByDeveloperId(this.userDev.id)
      .subscribe((response: any) => {
        this.digitalProfile = response;
        console.log(
          'Digital Profile Into Languages: ' +
            JSON.stringify(this.digitalProfile)
        );
      });
    var req = new XMLHttpRequest();
    req.open(
      'GET',
      `http://localhost:8080/api/v1/digital_profiles//developer/${this.userDev.id}`,
      false
    );
    req.send(null);
  ...
  AddDigitalProfile() {
    ...
    req.open(
      'GET',
      `http://localhost:8080/api/v1/users/searchByEmail/${
    ...
    if (req.status == 200) {
      ...
      this.digitalProfile.name =
        'Digital Profile ' + this.registerForm.get('first_name')?.value;
      var req2 = new XMLHttpRequest();
      req2.open(
        'POST',
        `http://localhost:8080/api/v1/digital_profiles/${this.userDev.id}`,
        false
      );
    ...
    if (this.registerForm.invalid) {
      ...
      if (!this.registered) {
        ...
        this.dialog.open(DialogBoxInvalidFormComponent, {
          data: {
            message:
              'You have registered successfully! You will be redirect to login page to Log In',
          },
        });
  ...
  MustMatch(password: any, password_confirm: any) {
    return (formGroup: FormGroup) => {
      ...
      if (
        ...
        !passwordConfirmControl.errors['mustMatch']
      ) {
        return;
      }
